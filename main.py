...
font_size = 35
font = pygame.font.SysFont("Times New Roman", font_size)  # Changed to Arial as it's more widely available
...
def load_image(image_name, new_width=screen_width, new_height=screen_height):
    try:
        image_path = f'{image_name}'  # Adjust the path as needed
        image = pygame.image.load(image_path)
        image = pygame.transform.scale(image, (new_width, new_height))
        return image
    except Exception as e:
        print(f"Failed to load and resize image {image_name}: {e}")
        return pygame.Surface((new_width, new_height)) 
...
async def main():
    ...
    while True:
        ...
        text_lines = render_text(texts[state], screen_width - 100)
        box_width = screen_width - 100
        box_height = len(text_lines) * font_size + 20
